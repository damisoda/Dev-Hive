...
- `auto_hkg.py` — **Auto-HKG v2 (2패스, HIVE-44)**. v1의 per-content 그리디 매칭(자동노드 825개·94% 고아)을 폐기하고: 1패스 대주제 흡수(코사인 ≥ 0.70) → 2패스 잔여 클러스터링(코사인 ≥ 0.65, MIN 3 연결요소) → 승격 클러스터만 Haiku로 네이밍. 실측(1283건, 2026-06-28): 자동노드 92개 · orphan_ratio 0.033 · depth 1
...
산출된 256차원 임베딩은 `content.graph_embedding`(vector 256)에 적재하도록 설계됐으나,
**2026-06-26 전체 재구축 이후 재export하지 않아 현재 0/1283로 미적재**이며, GraphRAG는
`graph_embedding`을 읽지 않아 **추천에 반영되지 않는다(future work)**.
