...
def test_template_reason_shows_interest_only_when_real():
    # profile 기반 관련성이 실제 계산 + 충분 → 관심 문구 등장(설명 문장)
    r = _template_reason({"diff": 0.8, "rel": 0.7, "rel_real": True})
    assert "관심" in r
    # quality 폴백(rel_real 아님, 신호 약함) → 관심 표기 안 함(오표기 방지)
    r2 = _template_reason({"diff": 0.4, "rel": 0.6, "rel_real": False})
    assert "관심" not in r2
